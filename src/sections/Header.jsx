...
function Header() {
  ...
  const NavLink =({title}) =>(
    <LinkScroll 
    to={title} offset={-100} onClick={()=>setIsOpen(false)}
    spy smooth activeClass="nav-active"
  ...
  return (
    ...
    hasScrolled &&
    "bg-black-100 py-2 backdrop-blur-[8px]")}>
      ...
      <div className={clsx(`w-full max-lg:fixed max-lg:top-0 
        ...
        max-lg:flex-col max-lg:min-h-screen max-lg:p-6 
        max-lg:overflow-hidden sidebar-before max-md:px-4 ">
          ...
          top-1/2 left-0 w-[960px] h-[380] translate-x-[-200px] -translate-y-1/2 rotate-90">
      ...
      border-s4/25 rounded-full flex justify-center items-center" onClick={()=>setIsOpen(prev=>!prev)}>
        <img src={`/images/${isOpen?"close":"magic"}.svg`} 
        alt="menu" className="size-1/2 object-contain" />
      </button>
    </div>
    </header>
  )
